    ...
    async def middle_main_step(self, unit, friends, foes):   #commandcenter 에는 A인지 E인지 넣기
        ...
        if self.bot.step_manager.step > 230:
            ...
            if unit.type_id == UnitTypeId.MARAUDER:
                ...
                if self.state == 'ready':
                    if unit.distance_to(gather)>3:
                        actions.append(unit.move(gather))
                    elif len(unit.passengers) <8:    # 탑승인원의 최대치보다 적으면
                        assult_units = self.bot.units.filter(lambda u: u.tag in self.unit_tags).of_type((UnitTypeId.MARINE, UnitTypeId.MARAUDER, 
    UnitTypeId.SIEGETANK, UnitTypeId.SIEGETANKSIEGED))
                        if self.bot.debug:
                            text=[
                            f'assult_unit \n'

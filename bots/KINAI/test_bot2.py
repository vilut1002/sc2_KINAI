...
class DummyBot(sc2.BotAI):
    """
    test_bot1_test 에서 turret kill 하는거 추가
    이제 합치는거만 남음?
    """
    isTankDropTiming = True    #탱크드랍 타이밍 재는 곳
    ...
    def __init__(self, debug=False, *args, **kwargs):
        ...
        self.turret_killer_manager = CombatGroupManager(self, Tactics.TURRET_KILLER)
        self.tank_drop_manager = CombatGroupManager(self, Tactics.TANK_DROP)
    ...
    def on_start(self):
        ...
        self.turret_killer_manager.reset()
    ...
    async def on_step(self, iteration: int):
        ...
        if self.step_manager.step % 2 == 0:
            ...
            # 새로운 공격지점 결정
            self.turret_killer_manager.target = self.terrein_manager.turret_point()            
            ...
            # 메인부터 방어지점 결정
            self.middle_main_manager.hold_point = self.terrein_manager.main_hold_point()

            

            # 부대 구성 변경
            # self.assign_manager.step()
            self.assign_manager.assign(self.turret_killer_manager)
            self.assign_manager.assign(self.sight_manager)
            self.assign_manager.assign(self.tank_drop_manager)      #혹시 순서가 차이가 있을 까봐 먼저 놓아봄
        ...
        actions = list()
        actions += await self.turret_killer_manager.step()
...
class CombatGroupManager(object):
    ...
    def __init__(self, bot_ai, tactics):
        self.bot = bot_ai
        self.strategy = None
        self.target = None      #공격 목표지점
        self.hold_point = None  #대기 지점
        self.unit_tags = None
        # 그룹의 경계 범위
    ...
    async def step(self):
        ...
        for unit in units:
            ...
            elif self.tactics == Tactics.TANK_DROP:
                actions += await self.tank_drop_step(unit, units, enemy)
    ...
    async def tank_drop_step(self,unit,friends,foes):
        actions = list()        
            ...
            if unit.type_id == UnitTypeId.MEDIVAC:
                ...
                if self.state == 'ready':
                    if len(unit.passengers) <2:    # 불곰 2마리를 아직 안태웠으면
                        friends_units = self.bot.units.of_type((UnitTypeId.MARAUDER))             
        ...
        if self.bot.step_manager.step > 230:
            ...
            if unit.type_id == UnitTypeId.MARAUDER:
                ...
                if self.state == 'ready':
                    ...
                    if self.bot.Strategy == 'SURPRISE':
                        if unit.distance_to(self.target) > 5:    #적 공터으로 처들어감 
                            actions.append(unit.move(self.target))    
                        else:
                            # 목표지점 공격으로 target 좌표 바꿔야함
                            actions.append(unit.stop())
                            self.state = 'combat'
                    else :
                        actions.append(unit.move(self.hold_point))  # 대기 지점으로 이동
...
class AssignManager():
    ...
    def assign(self, manager):
        ...
        if manager.tactics is Tactics.TURRET_KILLER:
            ...
            unit_tags = unit_tags - self.bot.turret_killer_manager.unit_tags
            unit_tags = unit_tags - self.bot.sight_manager.unit_tags
            unit_tags = unit_tags - self.bot.tank_drop_manager.unit_tags
            manager.unit_tags = unit_tags
            ...
            if self.bot.isTankDropTiming:   #지금이 tankdrop 할 타이밍인지 체크
                if manager.unit_tags == None:
                    group_units_tags = set()
                else:
                    group_units_tags = manager.unit_tags
        ...
        elif manager.tactics is Tactics.MIDDLE_TANK:    # 가운데 탱크 박는 부대
        ...
        elif manager.tactics is Tactics.POINT_DEFENSE or manager.tactics is Tactics.LAND_DEFENSE:  # 거점 방어와 공터 방어
            # 마린 3 불곰1 사신 1 의료선 1
            # 현재 지도에 존재하는 그룹 유닛
            group_units_tags = self.bot.units.tags & manager.unit_tags
            group_units = self.bot.units.filter(lambda u: u.tag in group_units_tags)
...
class StrategicManager(object):
    ...
    def step(self):
        if self.bot.supply_cap>=100 and self.bot.supply_used>= 60 :
            ...
            self.strategy = Strategy.SURPRISE
